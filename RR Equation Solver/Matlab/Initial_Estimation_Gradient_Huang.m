function centroid = Initial_Estimation_Gradient_Huang(z, K)
...
%   - If no feasible region is found, centroid is returned as NAN.
    ...
    %--------------------------------------------------------------
    % Step 5: Compute the centroid of the feasible-region vertices based on
    % gradient
    %--------------------------------------------------------------
    if isempty(vertices)
        ...
        weight = (1./grad)/sum(1./grad);
        centroid = (weight*vertices)';
    end
end